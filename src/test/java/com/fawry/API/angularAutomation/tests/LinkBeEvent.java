...
import com.fawry.API.angularAutomation.constants.excelIndices.LinkBeWithEventExcellndices;
...
import com.fawry.API.angularAutomation.pages.LinkBeEventPage;
...
public class LinkBeEvent extends BaseTest{
    ApiLogin LgnObj = new ApiLogin();
    String token = LgnObj.GetloginToken();

    @Test(description = "LinkBEwithEvent", dataProvider = "LinkBEwithEvent", priority = 0)
    public void LinkBEwithEvent(LinkBeEventDm LinkBEDMObj) {
        try {
            ...
            try {
                ...
                LinkBeEventPage EventPageobj = new LinkBeEventPage();
                EventPageobj.AddOJsonObjPayload(LinkBEDMObj,Payload);
                String endpoint = "/"+BE_ID+"/applications/event";
                ...
                if(ActualStatusCode==200 &&ActualStatusCode==ExpectedStatusCode){
                    ...
                    String GetFeaturesAPiName = "event";
    ...
    @DataProvider(name = "LinkBEwithEvent")
    public Object[][] provideLinkBEwithEvent(Method method) {
        ArrayList<ArrayList<Object>> resultArray = provideTestData("LinkBEwithEvent");
        Object[][] result = new Object[resultArray.size()][1];
        for (int i = 0; i < resultArray.size(); i++) {
            LinkBeEventDm linkBusinessEntityAPITestData = new LinkBeEventDm();
            linkBusinessEntityAPITestData.setTestCaseTitle(resultArray.get(i).get(LinkBeWithEventExcellndices.TEST_CASE_TITLE_INDEX).toString());
            linkBusinessEntityAPITestData.setApp_event(resultArray.get(i).get(LinkBeWithEventExcellndices.app_event).toString());
            linkBusinessEntityAPITestData.setEvent(resultArray.get(i).get(LinkBeWithEventExcellndices.event).toString());
            linkBusinessEntityAPITestData.setEvent_category(resultArray.get(i).get(LinkBeWithEventExcellndices.event_category).toString());
            linkBusinessEntityAPITestData.setStatus_Code(parseInt(resultArray.get(i).get(LinkBeWithEventExcellndices.ExpectedStatusCode).toString()));
            linkBusinessEntityAPITestData.setBE_Name(resultArray.get(i).get(LinkBeWithEventExcellndices.BE_name).toString());
            linkBusinessEntityAPITestData.setError_Code(resultArray.get(i).get(LinkBeWithEventExcellndices.Error_Code).toString());
