...
import com.fawry.API.angularAutomation.constants.excelIndices.LinkBeWithComplainExcellndices;
...
import com.fawry.API.angularAutomation.pages.LinkBeComplainPage;
...
public class LinkBeComplain extends BaseTest{
    ApiLogin LgnObj = new ApiLogin();
    String token = LgnObj.GetloginToken();

    public LinkBeComplain(){
  }

    @Test(description = "LinkBEwithComplain", dataProvider = "LinkBEwithComplain", priority = 0)
    public void LinkBEwithComplain(LinkBeComplainDm LinkBEDMObj) {
        try {
/*            extent.attachReporter(htmlReporter);
            ...
            try {
                ...
                LinkBeComplainPage ComplainPageobj = new LinkBeComplainPage();
                ComplainPageobj.AddOJsonObjPayload(LinkBEDMObj,Payload);
                String endpoint = "/"+BE_ID+"/applications/complaint";
                ...
                if(ActualStatusCode==200 &&ActualStatusCode==ExpectedStatusCode){
                    ...
                    String GetFeaturesAPiName = "complaint";
    ...
    @DataProvider(name = "LinkBEwithComplain")
    public Object[][] provideAddBusinessEntityAPI(Method method) {
        ArrayList<ArrayList<Object>> resultArray = provideTestData("LinkBEwithComplain");
        Object[][] result = new Object[resultArray.size()][1];
        for (int i = 0; i < resultArray.size(); i++) {
            LinkBeComplainDm linkBusinessEntityAPITestData = new LinkBeComplainDm();
            linkBusinessEntityAPITestData.setTestCaseTitle(resultArray.get(i).get(LinkBeWithComplainExcellndices.TEST_CASE_TITLE_INDEX).toString());
            linkBusinessEntityAPITestData.setApp_complaint(resultArray.get(i).get(LinkBeWithComplainExcellndices.app_complaint).toString());
            linkBusinessEntityAPITestData.setStatus_Code(parseInt(resultArray.get(i).get(LinkBeWithComplainExcellndices.ExpectedStatusCode).toString()));
            linkBusinessEntityAPITestData.setBE_Name(resultArray.get(i).get(LinkBeWithComplainExcellndices.BE_name).toString());
