...
public class LinkBeUsers extends BaseTest{
    ApiLogin LgnObj = new ApiLogin();
    String token = LgnObj.GetloginToken();

    @Test(description = "LinkBeWithUsers", dataProvider = "LinkBeWithUsers", priority = 0)
    public void LinkBEwithUsers(LinkBeUsersDM LinkBEDMObj) {
        try {
            ...
            try {
                ...
                LinkBeUsersPage UsersPageobj = new LinkBeUsersPage();
                UsersPageobj.AddOJsonObjPayload(LinkBEDMObj,Payload);
                String endpoint = "/"+BE_ID+"/applications/um";
    ...
    @DataProvider(name = "LinkBeWithUsers")
    public Object[][] provideAddBusinessEntityAPI(Method method) {
        ArrayList<ArrayList<Object>> resultArray = provideTestData("LinkBeWithUsers");
        Object[][] result = new Object[resultArray.size()][1];
        for (int i = 0; i < resultArray.size(); i++) {
            LinkBeUsersDM linkBusinessEntityAPITestData = new LinkBeUsersDM();
            linkBusinessEntityAPITestData.setTestCaseTitle(resultArray.get(i).get(LinkBeWithUsersExcellndices.TEST_CASE_TITLE_INDEX).toString());
            linkBusinessEntityAPITestData.setApp_user(resultArray.get(i).get(LinkBeWithUsersExcellndices.app_user).toString());
            linkBusinessEntityAPITestData.setUser(resultArray.get(i).get(LinkBeWithUsersExcellndices.user).toString());
            linkBusinessEntityAPITestData.setUser_role(resultArray.get(i).get(LinkBeWithUsersExcellndices.user_role).toString());
            linkBusinessEntityAPITestData.setIntegrationUrl(resultArray.get(i).get(LinkBeWithUsersExcellndices.integrationUrl).toString());
            linkBusinessEntityAPITestData.setStatus_Code(parseInt(resultArray.get(i).get(LinkBeWithUsersExcellndices.ExpectedStatusCode).toString()));
            linkBusinessEntityAPITestData.setBE_Name(resultArray.get(i).get(LinkBeWithUsersExcellndices.BE_name).toString());
            linkBusinessEntityAPITestData.setError_Code(resultArray.get(i).get(LinkBeWithUsersExcellndices.error_code).toString());
