...
public class UpdateBusinessEntityAPI extends BaseTest {

    ApiLogin LgnObj = new ApiLogin();
    String token = LgnObj.GetloginToken();

    @Test(description = "Update Business entity", dataProvider = "UpdateBeAPI", priority = 0)
    public void updateBusinesEntityApi(AddUpdateBusinessEntityDM UpdateBEObj) {


        try {
            test = extent.createTest(UpdateBEObj.getTestCaseTitle());
            Log.test = test;
            Log.startTestCase(UpdateBEObj.getTestCaseTitle());
            try {
                String Email = generalCofigsProps.getProperty(GeneralConstants.BE_MAIL_Update);
                ...
                if (BeIdDb != 0) {
                    ...
                    Response response = this.UpdateBusinessEntity(UpdateBEObj,BusinessEntityInDBbeforeubdate, BeIdDb);
                    int ExpectedStatusCode = UpdateBEObj.getStatus_Code();
    ...
    public Response UpdateBusinessEntity(AddUpdateBusinessEntityDM AddBEObj, AddUpdateBusinessEntityDM DataBaseObj, int BE_ID) {
        ...
        Log.info("sending Update api to http://10.95.0.178/be-api/business-entities");
        Log.info("sending Update api " + payload);

        Response response = given().header("Authorization", "Bearer " + token).
                contentType("application/json\r\n").body(payload).when().log()
                .all().put("");
        Log.info("Update API response with " + response.getBody().asString());
        return response;
    }

    @DataProvider(name = "UpdateBeAPI")
    public Object[][] provideAddBusinessEntityAPI(Method method) {
        ArrayList<ArrayList<Object>> resultArray = provideTestData("UpdateBeAPI");
       ...
       /*     } catch (NumberFormatException nfe) {
                System.out.println("the entry is empty with " + nfe);
            }*/
            // addBusinessEntityAPITestData.setChannels_MOB_id(parseInt(resultArray.get(i).get(AddUpdateBusinessEntityExcellndices.Channels_MOB_id).toString()));
