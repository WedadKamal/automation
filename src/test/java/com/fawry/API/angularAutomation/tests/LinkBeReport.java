...
import com.fawry.API.angularAutomation.constants.excelIndices.LinkBeWithReportExcellindices;
import com.fawry.API.angularAutomation.constants.excelIndices.LinkBeWithUsersExcellndices;
...
import com.fawry.API.angularAutomation.pages.LinkBeReportsPage;
import com.fawry.API.angularAutomation.pages.LinkBeUsersPage;
...
public class LinkBeReport extends BaseTest{

    ApiLogin LgnObj = new ApiLogin();
    String token = LgnObj.GetloginToken();

    @Test(description = "LinkBeWithReport", dataProvider = "LinkBeWithReport", priority = 0)
    public void LinkBeWithReport(LinkBeReportsDM LinkBEDMObj) {
        try {
            ...
            try {
                ...
                LinkBeReportsPage UsersPageobj = new LinkBeReportsPage();
                UsersPageobj.AddOJsonObjPayload(LinkBEDMObj,Payload);
                String endpoint = "/"+BE_ID+"/applications/report";
                ...
                if(ActualStatusCode==200 &&ActualStatusCode==ExpectedStatusCode){
                    ...
                    String GetFeaturesAPiName = "report";
    ...
    @DataProvider(name = "LinkBeWithReport")
    public Object[][] provideLinkBeWithReport(Method method) {
        ArrayList<ArrayList<Object>> resultArray = provideTestData("LinkBeWithReport");
        Object[][] result = new Object[resultArray.size()][1];
        for (int i = 0; i < resultArray.size(); i++) {
            LinkBeReportsDM linkBusinessEntityAPITestData = new LinkBeReportsDM();
            linkBusinessEntityAPITestData.setTestCaseTitle(resultArray.get(i).get(LinkBeWithReportExcellindices.TEST_CASE_TITLE_INDEX).toString());
            linkBusinessEntityAPITestData.setApp_report(resultArray.get(i).get(LinkBeWithReportExcellindices.app_report).toString());
            linkBusinessEntityAPITestData.setReport_event(resultArray.get(i).get(LinkBeWithReportExcellindices.report_event).toString());
            linkBusinessEntityAPITestData.setBranches_reports(resultArray.get(i).get(LinkBeWithReportExcellindices.branches_reports).toString());
            linkBusinessEntityAPITestData.setBranch_shifts_reports(resultArray.get(i).get(LinkBeWithReportExcellindices.branch_shifts_reports).toString());
            linkBusinessEntityAPITestData.setStatus_Code(parseInt(resultArray.get(i).get(LinkBeWithReportExcellindices.ExpectedStatusCode).toString()));
            linkBusinessEntityAPITestData.setBE_Name(resultArray.get(i).get(LinkBeWithReportExcellindices.BE_name).toString());
            linkBusinessEntityAPITestData.setError_Code(resultArray.get(i).get(LinkBeWithReportExcellindices.error_code).toString());
